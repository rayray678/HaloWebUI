...
LITE_PRESET = os.getenv("HALOWEBUI_LITE_PRESET", "").strip().lower()
LITE_PRESET_DEFAULTS = {
    "free-lite": {
        "CONTENT_EXTRACTION_ENGINE": "",
        "RAG_EMBEDDING_ENGINE": "openai",
        "RAG_EMBEDDING_MODEL": "jina-embeddings-v5-text-small",
...
class AppConfig:
    ...
    def __setattr__(self, key, value):
        if isinstance(value, PersistentConfig):
            ...
            if self._redis:
                redis_key = f"{REDIS_KEY_PREFIX}config:{key}"
...
OAUTH_REQUEST_TIMEOUT = int(os.environ.get("OAUTH_REQUEST_TIMEOUT", "30"))

OAUTH_TOKEN_ENDPOINT_AUTH_METHOD = os.environ.get(
...
OAUTH_TOKEN_EXCHANGE_ISSUER = os.environ.get("OAUTH_TOKEN_EXCHANGE_ISSUER", "")
OAUTH_TOKEN_EXCHANGE_JWKS_URI = os.environ.get("OAUTH_TOKEN_EXCHANGE_JWKS_URI", "")
OAUTH_TOKEN_EXCHANGE_AUDIENCE = os.environ.get("OAUTH_TOKEN_EXCHANGE_AUDIENCE", "")
...
OPENID_END_SESSION_ENDPOINT = os.environ.get("OPENID_END_SESSION_ENDPOINT", "")
...
def load_oauth_providers():
        ...
        def microsoft_oauth_register(client):
            client.register(
                ...
                client_kwargs={
                    ...
                    # Microsoft requires scope in the token refresh request body;
                    # client_secret_post ensures all params (including scope) are
                    # sent as form data, preventing AADSTS90009 errors.
    ...
    # Feishu / Lark SSO
    if FEISHU_APP_ID.value and FEISHU_APP_SECRET.value:
        ...
        def feishu_oauth_register(client):
            client.register(
                ...
                client_kwargs={"scope": "contact:user.base:readonly"},
...
ENABLE_BASE_MODELS_CACHE = PersistentConfig(
    ...
    "models.base_models_cache.enable",
...
# Normalize single GEMINI_API_BASE_URL env (legacy)
if GEMINI_API_BASE_URL != "" and GEMINI_API_BASE_URL.endswith("/"):
    GEMINI_API_BASE_URL = GEMINI_API_BASE_URL[:-1]

_effective_gemini_base_url = GEMINI_API_BASE_URL or DEFAULT_GEMINI_API_BASE_URL
...
GEMINI_API_BASE_URLS = (
    GEMINI_API_BASE_URLS if GEMINI_API_BASE_URLS != "" else _effective_gemini_base_url
)

GEMINI_API_BASE_URLS = [
    url.strip() if url.strip() != "" else DEFAULT_GEMINI_API_BASE_URL
    for url in GEMINI_API_BASE_URLS.split(";")
]
GEMINI_API_BASE_URLS = PersistentConfig(
    "GEMINI_API_BASE_URLS",
    "gemini.api_base_urls",
    GEMINI_API_BASE_URLS,
)
...
ANTHROPIC_API_BASE_URL = os.environ.get("ANTHROPIC_API_BASE_URL", "").rstrip("/")
...
ANTHROPIC_API_BASE_URLS = [
    url.strip() if url.strip() != "" else DEFAULT_ANTHROPIC_API_BASE_URL
    for url in ANTHROPIC_API_BASE_URLS.split(";")
]
ANTHROPIC_API_BASE_URLS = PersistentConfig(
    "ANTHROPIC_API_BASE_URLS",
    "anthropic.api_base_urls",
    ANTHROPIC_API_BASE_URLS,
)
...
# Global default for tool calling. This is only applied when a chat/model does not
# explicitly set params.function_calling.
TOOL_CALLING_MODE = PersistentConfig(
    "TOOL_CALLING_MODE",
    "tools.calling.mode",
    os.environ.get("TOOL_CALLING_MODE", "default").strip().lower(),
)

# When disabled, native tool calling will execute one tool-call round and then
# ask the model to synthesize the final answer without further tool calls.
ENABLE_INTERLEAVED_THINKING = PersistentConfig(
    "ENABLE_INTERLEAVED_THINKING",
    "tools.calling.interleaved_thinking.enable",
    os.environ.get("ENABLE_INTERLEAVED_THINKING", "False").lower() == "true",
)

# Custom reasoning/thinking tags for streaming detection.
# JSON-encoded list of [open_tag, close_tag] pairs.
# When empty, uses built-in defaults (think, thinking, reason, reasoning, etc.)
CUSTOM_REASONING_TAGS = PersistentConfig(
    "CUSTOM_REASONING_TAGS",
    "reasoning.custom_tags",
    os.environ.get("CUSTOM_REASONING_TAGS", ""),
)

# Fine-grained gates for builtin/system tools injected in Native Mode.
# NOTE: Some tools also require their underlying feature to be enabled
# (e.g. web search, image generation, channels).
ENABLE_WEB_SEARCH_TOOL = PersistentConfig(
    "ENABLE_WEB_SEARCH_TOOL",
    "tools.builtin.web.search.enable",
    os.environ.get("ENABLE_WEB_SEARCH_TOOL", "True").lower() == "true",
)
ENABLE_URL_FETCH = PersistentConfig(
    "ENABLE_URL_FETCH",
    "tools.builtin.fetch_url.enable",
    os.environ.get("ENABLE_URL_FETCH", "True").lower() == "true",
)
ENABLE_URL_FETCH_RENDERED = PersistentConfig(
    "ENABLE_URL_FETCH_RENDERED",
    "tools.builtin.fetch_url_rendered.enable",
    os.environ.get("ENABLE_URL_FETCH_RENDERED", "False").lower() == "true",
)

# Knowledge tools
ENABLE_LIST_KNOWLEDGE_BASES = PersistentConfig(
    "ENABLE_LIST_KNOWLEDGE_BASES",
    "tools.builtin.knowledge.list_bases.enable",
    os.environ.get("ENABLE_LIST_KNOWLEDGE_BASES", "True").lower() == "true",
)
ENABLE_SEARCH_KNOWLEDGE_BASES = PersistentConfig(
    "ENABLE_SEARCH_KNOWLEDGE_BASES",
    "tools.builtin.knowledge.search_bases.enable",
    os.environ.get("ENABLE_SEARCH_KNOWLEDGE_BASES", "True").lower() == "true",
)
ENABLE_QUERY_KNOWLEDGE_FILES = PersistentConfig(
    "ENABLE_QUERY_KNOWLEDGE_FILES",
    "tools.builtin.knowledge.query_files.enable",
    os.environ.get("ENABLE_QUERY_KNOWLEDGE_FILES", "True").lower() == "true",
)
ENABLE_VIEW_KNOWLEDGE_FILE = PersistentConfig(
    "ENABLE_VIEW_KNOWLEDGE_FILE",
    "tools.builtin.knowledge.view_file.enable",
    os.environ.get("ENABLE_VIEW_KNOWLEDGE_FILE", "True").lower() == "true",
)

# Image tools
ENABLE_IMAGE_GENERATION_TOOL = PersistentConfig(
    "ENABLE_IMAGE_GENERATION_TOOL",
    "tools.builtin.images.generate.enable",
    os.environ.get("ENABLE_IMAGE_GENERATION_TOOL", "True").lower() == "true",
)
ENABLE_IMAGE_EDIT = PersistentConfig(
    "ENABLE_IMAGE_EDIT",
    "tools.builtin.images.edit.enable",
    os.environ.get("ENABLE_IMAGE_EDIT", "False").lower() == "true",
)

# Memory tools
ENABLE_MEMORY_TOOLS = PersistentConfig(
    "ENABLE_MEMORY_TOOLS",
    "tools.builtin.memory.enable",
    os.environ.get("ENABLE_MEMORY_TOOLS", "True").lower() == "true",
)

# Notes tools (experimental)
ENABLE_NOTES = PersistentConfig(
    "ENABLE_NOTES",
    "tools.builtin.notes.enable",
    os.environ.get("ENABLE_NOTES", "False").lower() == "true",
)

# Chat history tools
ENABLE_CHAT_HISTORY_TOOLS = PersistentConfig(
    "ENABLE_CHAT_HISTORY_TOOLS",
    "tools.builtin.chats.enable",
    os.environ.get("ENABLE_CHAT_HISTORY_TOOLS", "True").lower() == "true",
)

# Time tools
ENABLE_TIME_TOOLS = PersistentConfig(
    "ENABLE_TIME_TOOLS",
    "tools.builtin.time.enable",
    os.environ.get("ENABLE_TIME_TOOLS", "True").lower() == "true",
)

# Channels tools
ENABLE_CHANNEL_TOOLS = PersistentConfig(
    "ENABLE_CHANNEL_TOOLS",
    "tools.builtin.channels.enable",
    os.environ.get("ENABLE_CHANNEL_TOOLS", "True").lower() == "true",
)
...
# Default sharing permission for groups: "none" / "members" / "anyone"
DEFAULT_GROUP_SHARE_PERMISSION = PersistentConfig(
    "DEFAULT_GROUP_SHARE_PERMISSION",
    "group.default_share_permission",
    os.getenv("DEFAULT_GROUP_SHARE_PERMISSION", "members"),
)

# Default sharing permission for groups auto-created via OAuth
OAUTH_GROUP_DEFAULT_SHARE = PersistentConfig(
    "OAUTH_GROUP_DEFAULT_SHARE",
    "group.oauth_default_share",
    os.getenv("OAUTH_GROUP_DEFAULT_SHARE", "members"),
)
...
Based on the chat history, generate 3 concise follow-up questions that the user might want to ask next.

### Guidelines:
- Questions should be relevant to the conversation topic and naturally continue the discussion.
- Each question should explore a different aspect or direction.
- Keep questions concise (under 20 words each).
- Use the same language as the conversation.
- Do NOT repeat questions already asked.

### Output:
Respond **EXCLUSIVELY** with a JSON object. Do NOT include any additional text, explanation, or formatting.
Format: { "follow_ups": ["question1", "question2", "question3"] }
...
1. **Code Interpreter**: `<code_interpreter type="code" lang="python"></code_interpreter>`
   ...
   - When you create any downloadable output file, save it using a relative filename or path and print a Markdown link to that file, for example `[filename.ext](filename.ext)`. This applies to all file types, not only CSV.
...
ENABLE_TERMINAL = PersistentConfig(
    "ENABLE_TERMINAL",
    "terminal.enable",
    os.environ.get("ENABLE_TERMINAL", "False").lower() == "true",
)

TERMINAL_WORKSPACE_DIR = PersistentConfig(
    "TERMINAL_WORKSPACE_DIR",
    "terminal.workspace_dir",
    os.environ.get("TERMINAL_WORKSPACE_DIR", ""),
)

TERMINAL_COMMAND_TIMEOUT = PersistentConfig(
    "TERMINAL_COMMAND_TIMEOUT",
    "terminal.command_timeout",
    int(os.environ.get("TERMINAL_COMMAND_TIMEOUT", "30")),
)

TERMINAL_MAX_OUTPUT_CHARS = PersistentConfig(
    "TERMINAL_MAX_OUTPUT_CHARS",
    "terminal.max_output_chars",
    int(os.environ.get("TERMINAL_MAX_OUTPUT_CHARS", "65536")),
)
...
PGVECTOR_INDEX_LISTS = int(os.environ.get("PGVECTOR_INDEX_LISTS", "100"))
...
RAG_HYBRID_SEARCH_BM25_WEIGHT = PersistentConfig(
    ...
    float(os.environ.get("RAG_HYBRID_SEARCH_BM25_WEIGHT", "0.5")),
)
...
RAG_EMBEDDING_ENGINE = PersistentConfig(
    ...
    preset_env("RAG_EMBEDDING_ENGINE", ""),
)
...
PDF_LOADING_MODE = PersistentConfig(
    "PDF_LOADING_MODE",
    "rag.pdf_loading_mode",
    os.environ.get("PDF_LOADING_MODE", ""),
)
...
RAG_EMBEDDING_CONCURRENT_REQUESTS = PersistentConfig(
    ...
    int(os.environ.get("RAG_EMBEDDING_CONCURRENT_REQUESTS", "5")),
)

RAG_RERANKING_MODEL = PersistentConfig(
    "RAG_RERANKING_MODEL",
    "rag.reranking_model",
    preset_env("RAG_RERANKING_MODEL", ""),
)
...
RAG_RERANKING_ENGINE = PersistentConfig(
    ...
    preset_env("RAG_RERANKING_ENGINE", "local"),
)
...
RAG_SYSTEM_CONTEXT = PersistentConfig(
    "RAG_SYSTEM_CONTEXT",
    "rag.system_context",
    os.environ.get("RAG_SYSTEM_CONTEXT", ""),
)

RAG_OPENAI_API_BASE_URL = PersistentConfig(
    "RAG_OPENAI_API_BASE_URL",
    "rag.openai_api_base_url",
    preset_env("RAG_OPENAI_API_BASE_URL", OPENAI_API_BASE_URL),
)
...
ENABLE_NATIVE_WEB_SEARCH = PersistentConfig(
    "ENABLE_NATIVE_WEB_SEARCH",
    "rag.web.search.native.enable",
...
JINA_API_BASE_URL = PersistentConfig(
    ...
    os.getenv("JINA_API_BASE_URL", "https://s.jina.ai/"),
)

DDGS_BACKEND = PersistentConfig(
    "DDGS_BACKEND",
    "rag.web.search.ddgs_backend",
    os.getenv("DDGS_BACKEND", "lite"),
)
...
GROK_API_KEY = PersistentConfig(
    "GROK_API_KEY",
    "rag.web.search.grok_api_key",
    os.getenv("GROK_API_KEY", ""),
)

GROK_API_BASE_URL = PersistentConfig(
    "GROK_API_BASE_URL",
    "rag.web.search.grok_api_base_url",
    os.getenv("GROK_API_BASE_URL", "https://api.x.ai"),
)

GROK_API_MODEL = PersistentConfig(
    "GROK_API_MODEL",
    "rag.web.search.grok_api_model",
    os.getenv("GROK_API_MODEL", "grok-4-1-fast"),
)

GROK_API_MODE = PersistentConfig(
    "GROK_API_MODE",
    "rag.web.search.grok_api_mode",
    os.getenv("GROK_API_MODE", "chat_completions"),
)
...
FIRECRAWL_TIMEOUT = PersistentConfig(
    ...
    int(os.environ.get("FIRECRAWL_TIMEOUT", "30")),
)
...
ENABLE_IMAGE_GENERATION_SHARED_KEY = PersistentConfig(
...
IMAGE_MODEL_FILTER_REGEX = PersistentConfig(
    "IMAGE_MODEL_FILTER_REGEX",
    "image_generation.model_filter_regex",
    os.getenv("IMAGE_MODEL_FILTER_REGEX", ""),
)
...
AUDIO_STT_OPENAI_API_BASE_URL = PersistentConfig(
    ...
    preset_env("AUDIO_STT_OPENAI_API_BASE_URL", OPENAI_API_BASE_URL),
)
...
AUDIO_TTS_OPENAI_API_BASE_URL = PersistentConfig(
    ...
    preset_env("AUDIO_TTS_OPENAI_API_BASE_URL", OPENAI_API_BASE_URL),
)
...
ENABLE_LDAP_GROUP_SYNC = PersistentConfig(
    "ENABLE_LDAP_GROUP_SYNC",
    "ldap.group_sync.enable",
    os.environ.get("ENABLE_LDAP_GROUP_SYNC", "false").lower() == "true",
)

LDAP_GROUP_ATTRIBUTE = PersistentConfig(
    "LDAP_GROUP_ATTRIBUTE",
    "ldap.group_sync.attribute",
    os.environ.get("LDAP_GROUP_ATTRIBUTE", "memberOf"),
)

LDAP_GROUP_SEARCH_BASE = PersistentConfig(
    "LDAP_GROUP_SEARCH_BASE",
    "ldap.group_sync.search_base",
    os.environ.get("LDAP_GROUP_SEARCH_BASE", ""),
)

LDAP_GROUP_SEARCH_FILTER = PersistentConfig(
    "LDAP_GROUP_SEARCH_FILTER",
    "ldap.group_sync.search_filter",
    os.environ.get("LDAP_GROUP_SEARCH_FILTER", "(objectClass=groupOfNames)"),
)
