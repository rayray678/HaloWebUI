...
from open_webui.runtime_migrations import ensure_runtime_migrated

ensure_runtime_migrated()
...
async def lifespan(app: FastAPI):
    ...
    # Auto-create admin account from env vars for headless deployments
    if WEBUI_ADMIN_EMAIL and WEBUI_ADMIN_PASSWORD:
        try:
            ...
            if Users.get_num_users() == 0:
                hashed = get_password_hash(WEBUI_ADMIN_PASSWORD)
                Auths.insert_new_auth(
                    WEBUI_ADMIN_EMAIL.lower(),
                    hashed,
                    WEBUI_ADMIN_NAME,
                    "admin",
                )
...
app.state.config.TOOL_CALLING_MODE = TOOL_CALLING_MODE
app.state.config.ENABLE_INTERLEAVED_THINKING = ENABLE_INTERLEAVED_THINKING
app.state.config.CUSTOM_REASONING_TAGS = CUSTOM_REASONING_TAGS

app.state.config.ENABLE_WEB_SEARCH_TOOL = ENABLE_WEB_SEARCH_TOOL
app.state.config.ENABLE_URL_FETCH = ENABLE_URL_FETCH
app.state.config.ENABLE_URL_FETCH_RENDERED = ENABLE_URL_FETCH_RENDERED

app.state.config.ENABLE_LIST_KNOWLEDGE_BASES = ENABLE_LIST_KNOWLEDGE_BASES
app.state.config.ENABLE_SEARCH_KNOWLEDGE_BASES = ENABLE_SEARCH_KNOWLEDGE_BASES
app.state.config.ENABLE_QUERY_KNOWLEDGE_FILES = ENABLE_QUERY_KNOWLEDGE_FILES
app.state.config.ENABLE_VIEW_KNOWLEDGE_FILE = ENABLE_VIEW_KNOWLEDGE_FILE

app.state.config.ENABLE_IMAGE_GENERATION_TOOL = ENABLE_IMAGE_GENERATION_TOOL
app.state.config.ENABLE_IMAGE_EDIT = ENABLE_IMAGE_EDIT

app.state.config.ENABLE_MEMORY_TOOLS = ENABLE_MEMORY_TOOLS
app.state.config.ENABLE_NOTES = ENABLE_NOTES
app.state.config.ENABLE_CHAT_HISTORY_TOOLS = ENABLE_CHAT_HISTORY_TOOLS
app.state.config.ENABLE_TIME_TOOLS = ENABLE_TIME_TOOLS
app.state.config.ENABLE_CHANNEL_TOOLS = ENABLE_CHANNEL_TOOLS
...
# confusion and reduce attack surface.
if app.state.config.ENABLE_DIRECT_CONNECTIONS:
    app.state.config.ENABLE_DIRECT_CONNECTIONS = False

# The base model list cache toggle is removed from the UI; keep it enabled for performance.
if not app.state.config.ENABLE_BASE_MODELS_CACHE:
    app.state.config.ENABLE_BASE_MODELS_CACHE = True
...
app.state.config.OAUTH_GROUP_DEFAULT_SHARE = OAUTH_GROUP_DEFAULT_SHARE
...
app.state.config.RAG_SYSTEM_CONTEXT = RAG_SYSTEM_CONTEXT
...
app.state.config.GROK_API_KEY = GROK_API_KEY
app.state.config.GROK_API_BASE_URL = GROK_API_BASE_URL
app.state.config.GROK_API_MODEL = GROK_API_MODEL
app.state.config.GROK_API_MODE = GROK_API_MODE
...
app.state.config.IMAGES_OPENAI_API_FORCE_MODE = IMAGES_OPENAI_API_FORCE_MODE
...
app.state.BASE_MODELS_LOCK = asyncio.Lock()
...
# API Rate Limiting Middleware (Redis-backed)
if ENABLE_API_RATE_LIMIT and REDIS_URL:
    _rate_limit_redis = None
    try:
        from open_webui.utils.redis import get_redis_connection

        _rate_limit_redis = get_redis_connection(
            REDIS_URL,
            get_sentinels_from_env(REDIS_SENTINEL_HOSTS, REDIS_SENTINEL_PORT),
            cluster_mode=REDIS_CLUSTER_MODE,
        )
        log.info(f"API rate limiting enabled: {API_RATE_LIMIT_RPM} RPM")
    except Exception as e:
        log.warning(f"Failed to connect Redis for rate limiting: {e}")
    ...
    if _rate_limit_redis:
        ...
        async def rate_limit_middleware(request: Request, call_next):
            ...
            path = request.url.path
            if not path.startswith("/api/") or "/ws/" in path:
                return await call_next(request)

            # Identify by user token or IP
            token = request.state.token
            if token:
                key = f"{REDIS_KEY_PREFIX}rate:{token[:32]}"
            else:
                client_ip = request.client.host if request.client else "unknown"
                key = f"{REDIS_KEY_PREFIX}rate:{client_ip}"

            try:
                count = _rate_limit_redis.incr(key)
                if count == 1:
                    _rate_limit_redis.expire(key, 60)
                ttl = _rate_limit_redis.ttl(key)

                if count > API_RATE_LIMIT_RPM:
                    return JSONResponse(
                        status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                        content={"detail": ERROR_MESSAGES.RATE_LIMIT_EXCEEDED},
                        headers={
                            "Retry-After": str(max(ttl, 1)),
                            "X-RateLimit-Limit": str(API_RATE_LIMIT_RPM),
                            "X-RateLimit-Remaining": "0",
                        },
                    )

                response = await call_next(request)
                response.headers["X-RateLimit-Limit"] = str(API_RATE_LIMIT_RPM)
                response.headers["X-RateLimit-Remaining"] = str(
                    max(0, API_RATE_LIMIT_RPM - count)
                )
                return response
            except Exception:
...
async def chat_completion(
    ...
    try:
        ...
        metadata = {
            ...
            **({"function_calling": "native"} if function_calling_native else {}),
        }
...
def _get_user_tool_calling_mode(request: Request, user) -> str:
    """Return the effective tool calling mode for the current user.

    Priority: user setting > system global default.
    """
    if user is not None:
        native_cfg = get_user_native_tools_config(request, user)
        if native_cfg:
            mode = normalize_tool_calling_mode(
                native_cfg.get(TOOL_CALLING_MODE_KEY),
            )
            return mode
            ...
            **(
                {
                    ...
                    "enable_web_search": bool(
                        app.state.config.ENABLE_WEB_SEARCH
        ...
        **(
            {
                ...
                "tools": {
                    "calling_mode": _get_user_tool_calling_mode(request, user),
                    "interleaved_thinking": app.state.config.ENABLE_INTERLEAVED_THINKING,
                },
                "reasoning": {
                    "custom_tags": app.state.config.CUSTOM_REASONING_TAGS,
                },
                "code": {
