            ...
            try:
                ...
                if r and r.status != 200:
                    ...
                    if "json" in content_type:
                        res = await r.json()
                        if "error" in res:
                            detail = f"Azure TTS: {res['error'].get('message', res['error'])}"
                    else:
                        body_text = await r.text()
                        detail = f"Azure TTS ({r.status}): {body_text[:200]}"
            except Exception:
                detail = f"Azure TTS: {e}"
...
def transcribe(request: Request, file_path, language: str = ""):
    ...
    if request.app.state.config.STT_ENGINE == "":
        ...
        segments, info = model.transcribe(file_path, **whisper_kwargs)
        log.info(
        ...
        try:
            with open(file_path, "rb") as audio_file:
                ...
                if language:
                    form_data_fields["language"] = language
                r = requests.post(
...
def transcription(
    ...
    try:
        ...
        try:
            ...
            data = transcribe(request, file_path, language=language)
