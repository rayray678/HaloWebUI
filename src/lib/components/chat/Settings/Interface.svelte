<script lang="ts">
	...
	const toggleSvgPreviewAutoOpen = async () => {
		svgPreviewAutoOpen = !svgPreviewAutoOpen;
		saveSettings({ svgPreviewAutoOpen });
	};
			...
			<div>
				<div class=" py-0.5 flex w-full justify-between">
					...
					<button
						class="p-1 px-3 text-xs flex rounded-sm transition"
						on:click={() => {
							toggleSvgPreviewAutoOpen();
						}}
						type="button"
					>
						{#if svgPreviewAutoOpen === true}
							<span class="ml-2 self-center">{$i18n.t('On')}</span>
						{:else}
							<span class="ml-2 self-center">{$i18n.t('Off')}</span>
						{/if}
